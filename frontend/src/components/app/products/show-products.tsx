...
export const ShowProducts = () => {
    ...
    useEffect(() => {
        if (categories) {
            setCategoriesSelected(categories.map(category => category.name));
        }
    }, [categories]);
            ...
            <div className="flex flex-col gap-2 p-4">
                ...
                <div className="flex items-center gap-2">
                    {categories && categories.map((category, index) => {
                        return (
                            <Badge key={index} variant={categoriesSelected.includes(category.name) ? "default" : "outline"} onClick={() => handleClickCategory(category)}>{category.name}</Badge>
                        )
                    })}
                </div>
            </div>
